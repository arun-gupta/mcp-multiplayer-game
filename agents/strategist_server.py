#!/usr/bin/env python3
"""
Standalone Strategist Agent MCP Server
Runs as independent process on port 3002
"""
import uvicorn
...
app = FastAPI(
    title="Strategist Agent MCP Server",
    description="Standalone Strategist Agent with MCP Protocol",
    version="1.0.0"
)
...
async def startup_event():
    ...
    print("🚀 Starting Strategist Agent MCP Server...")

    # Get default model
    available_models = ModelFactory.get_default_models()
    model = available_models.get("strategist", "gpt-5-mini")

    print(f"🔍 Initializing Strategist Agent with model: {model}")
    strategist_agent = StrategistMCPAgent({"model": model})

    # Start MCP server
    await strategist_agent.start_mcp_server()
    print(f"✅ Strategist Agent ready on port 3002")
    print(f"   MCP endpoint: http://localhost:3002/mcp")
...
async def health_check():
    ...
    return {
        ...
        "agent": "strategist",
        "port": 3002,
        "model": strategist_agent.__dict__.get('current_model') if strategist_agent else "unknown"
    }
...
async def mcp_discovery():
    ...
    if strategist_agent is None:
        raise HTTPException(status_code=503, detail="Strategist agent not initialized")

    # Get registries
    tools_registry = strategist_agent.__dict__.get('tools_registry', {})
    resources_registry = strategist_agent.__dict__.get('resources_registry', {})
    prompts_registry = strategist_agent.__dict__.get('prompts_registry', {})
...
if __name__ == "__main__":
    print("=" * 60)
    print("Strategist Agent MCP Server")
    print("=" * 60)
